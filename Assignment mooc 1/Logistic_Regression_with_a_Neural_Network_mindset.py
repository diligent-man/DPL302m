...
def sigmoid(z):
    ...
    return s
def sigmoid_test(sigmoid_function):
    z = np.array([0, 2])
...
def propagate_test(propagate_function):
    ...
    grads, cost = propagate_function(w, b, X, Y)
...
def optimize_test(optimize_function):
    # Create dummy data for testing
    w = np.array([[1.], [2.]])
    b = 1.5
    X = np.array([[1., -2., -1.], [3., 0.5, -3.2]])
...
def predict_test(predict_function):
    # Create dummy data for testing
    ...
    Y_prediction = predict_function(w, b, X)

...
def init_vars(src, model, SOURCE, TARGET, option):
    init_tok = TARGET.vocab.stoi['<sos>']
    src_mask = (src != SOURCE.vocab.stoi['<pad>']).unsqueeze(-2)
    e_output = model.encoder(src, src_mask)
    outputs = torch.LongTensor([[init_tok]])
    if option.cuda == True:
        outputs = outputs.to(option.cuda_device)
    trg_mask = no_peak_masks(1, option)
...
def beam_search(src, model, SOURCE, TARGET, option):
    outputs, e_outputs, log_scores = init_vars(src, model, SOURCE, TARGET, option)
    eos_tok = TARGET.vocab.stoi['<eos>']
    src_mask = (src != SOURCE.vocab.stoi['<pad>']).unsqueeze(-2)
    ind = None
    for i in range(2, option.max_strlen):
        trg_mask = no_peak_masks(i, option)
